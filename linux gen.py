    ...
    for x in filesinrootdir:
        if re.search("plugin|repository|script|skin|network|program", x):  # |repository
            ...
            for y in filesinfoldertozip:
                ...
                try:
                    if re.search("changelog", y):
                        firstpart = y[:-4]
                        lastpart = y[len(y) - 4:]
                        shutil.copyfile(os.path.join(rootdir, x, y), os.path.join(
                            zipsfolder, firstpart + version + lastpart))
                        print 'Copying ' + y + ' to ' + zipsfolder
                except:
                       pass
                if re.search("icon|fanart", y):
                    shutil.copyfile(os.path.join(rootdir, x, y),
                                    os.path.join(zipsfolder, y))
                    print 'Copying ' + y + ' to ' + zipsfolder
            zipfolder(zipfilenamefirstpart + zipfilenamelastpart,
                      foldertozip, zipsfolder)
            ...
            shutil.move(os.path.join(os.path.join(os.getcwd(), zipsfolder), zipfilenamefirstpart + zipfilenamelastpart),
                        os.path.join(os.path.join(os.getcwd(), zipsfolder), zipfilenamefirstpart + version + zipfilenamelastpart))
