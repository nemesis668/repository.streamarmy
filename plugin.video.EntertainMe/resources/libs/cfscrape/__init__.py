...
class CloudflareScraper(Session):
    ...
    def loadCipherSuite(self):
        ...
        raise RuntimeError("The SSL on this system does not meet the minimum cipher requirements.")
    ...
    def request(self, method, url, *args, **kwargs):
        ...
        resp = self.decodeBrotli(
            super(CloudflareScraper, self).request(method, url, *args, **kwargs)
        )
    ...
    @staticmethod
    def IUAM_Challenge_Response(body, url):
        try:
        ...
        try:
            payload['jschl_answer'] = utils.solve_cf_challenge(body, hostParsed.netloc)
    ...
    def Challenge_Response(self, resp, **kwargs):
        if self.is_reCaptcha_Challenge(resp):
            ...
            resp = self.decodeBrotli(
                super(CloudflareScraper, self).request(resp.request.method, resp.url, **kwargs)
            )
    ...
    # ------------------------------------------------------------------------------- #
    # Functions for integrating CloudflareScraper with other applications and scripts
    # ------------------------------------------------------------------------------- #
