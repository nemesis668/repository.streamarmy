...
def content(url,searched=False):
    ...
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'a', req=['href','title']), \
            dom_parser2.parse_dom(i, 'span', {'class': 'timer'}), \
            dom_parser2.parse_dom(i, 'img', req='src')) for i in r if i if 'class="thumb"' in i.content]
        r = [(urlparse.urljoin(base_domain,i[0][0].attrs['href']),i[0][0].attrs['title'],i[1][0].content,i[2][0].attrs['src']) for i in r if i]
    ...
    if not searched:
        search_pattern = '''<li><a class=['"]selected"\s*>\d+<\/a><\/li>\s*<li><a\s*href=['"]([^'"]+)['"]>\d+<\/a><\/li>'''
        parse = base_domain
