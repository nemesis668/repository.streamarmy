...
filename     = 'spreadporn'
base_domain  = 'http://spreadporn.org'
...
def content(url):
    ...
    try:
        if not url: url = 'http://spreadporn.org/category/movies/'
        c = client.request(url)
        r = dom_parser2.parse_dom(c, 'article')
        r = [(dom_parser2.parse_dom(i, 'a', req=['href','title']), \
              dom_parser2.parse_dom(i, 'img', req='class')) \
              for i in r if i]
        r = [(i[0][2].attrs['href'], i[0][2].attrs['title'], i[1][0].attrs['src']) for i in r]
        if ( not r ):
