...
base_domain  = 'https://www.vporn.com'
...
def menu():
    ...
    try:
        ...
        r = dom_parser2.parse_dom(c, 'div', {'class': ['tags-list','tag-limit']})
        r = dom_parser2.parse_dom(r, 'li')
        r = [dom_parser2.parse_dom(i, 'a', req=['href','title']) for i in r]
        r = [(urlparse.urljoin(base_domain, i[0].attrs['href']), i[0].content, i[0].attrs['title'].replace(' videos','')) for i in r if i]
        if ( not r ):
...
def content(url,searched=False):
    ...
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'a', req=['href','title']), \
            ...
            for i in r]
        r = [(i[0][0].attrs['href'], i[0][0].attrs['title'], i[1][0].content, i[2][0].attrs['src']) for i in r if i]
