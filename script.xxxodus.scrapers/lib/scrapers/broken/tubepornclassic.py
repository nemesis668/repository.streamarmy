...
base_domain  = 'https://fr.tubepornclassic.com'
...
def content(url,searched=False):
    ...
    try:
        c = client.request(url)
        r = dom_parser2.parse_dom(c, 'div', {'class': re.compile('ite.+?')})
        r = [(dom_parser2.parse_dom(i, 'a', req=['href','title']), \
              dom_parser2.parse_dom(i, 'div', {'class': 'duration'}), 
              re.findall('''<img.+?src=['"]([^'"]+)['"]''', i.content)[0]) \
             for i in r if i]
        r = [(i[0][0].attrs['href'], i[0][0].attrs['title'], i[1][0].content, i[2]) for i in r]
    ...
    if dirlst:
        if not searched:
            try:
                try:
                    pattern = '''(?:\/\/|\.)(?:tubepornclassic\.com)\/(?:search)\/(?:\w+)\/\?.+?from_videos=(\d+)'''
                    page = re.findall(pattern, url)[0]
                    np = int(page) + 1
                    url = url.replace('from_videos='+str(page),'from_videos='+str(np))
                except: 
                    url += '?mode=async&function=get_block&block_id=list_videos_videos_list_search_result&from_videos=2'
                dirlst.append({'name': 'Next Page -->', 'url': url, 'mode': content_mode, 'icon': xbmc.translatePath(os.path.join('special://home/addons/script.xxxodus.artwork', 'resources/art/main/next.png')), 'fanart': fanarts, 'description': 'Load More Videos...', 'folder': True})
            except: pass
            buildDirectory(dirlst, isVideo = True, isDownloadable = True)
        else: buildDirectory(dirlst, stopend=True, isVideo = True, isDownloadable = True)
    else:
        if (not searched):
