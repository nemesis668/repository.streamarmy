...
def menu():
	...
	try:
		...
		r = re.findall('<div class="container">(.*?)</section>',c,flags=re.DOTALL)[0]
		regex = '''<a\s+href=['"](.*?)['"].+?\s+.+?\s+.+src=['"]([^'"]+)['"].+?\s+.+?.+\s+.+\s+.+?>(.*?)<'''
...
def content(url,searched=False):
	...
	try:
		c = client.request(url)
		r = re.findall('<section\s+class="content">(.*?)</section>',c,flags=re.DOTALL)[0]
		r = re.findall('title=(.*?)class="squares__item js-video"',r,flags=re.DOTALL)
	...
	for i in r:
		try:
			title = re.findall('"(.*?)"',i,flags=re.DOTALL)[0]
			icon = re.findall('data-loader-src="(.*?)"',i,flags=re.DOTALL)[0]
