...
def menu():
    ...
    try:
        ...
        r = dom_parser2.parse_dom(c, 'div', {'class': re.compile('list\spage')})
        r = dom_parser2.parse_dom(r, 'a')
        r = [i for i in r if '<span' in i.content]
        r = [(i.attrs['href'], dom_parser2.parse_dom(i.content, 'span ')) for i in r]
        r = [(i[0], i[1][0].content) for i in r]
        if ( not r ):
...
def content(url,searched=False):
    ...
    try:
        c = client.request(url)
        r = dom_parser2.parse_dom(c, 'div', {'class': ['thumb-list__item','video-thumb']})
        r = [(dom_parser2.parse_dom(i, 'a', req='href'), \
            dom_parser2.parse_dom(i, 'div', {'class': 'thumb-image-container__duration'}), \
            dom_parser2.parse_dom(i, 'img', req='src')) \
            for i in r if i]
        r = [(i[0][-1].attrs['href'], i[0][-1].content, i[1][0].content, i[2][0].attrs['src']) for i in r]
