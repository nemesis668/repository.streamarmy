...
def content(url,searched=False):
	...
	for i in r:
		try:
			...
			icon = i.img['src']
			if not 'http' in icon: icon = 'https:'+icon
			content_url = url2 + '|SPLIT|%s' % base_name
			if searched: description = 'Result provided by %s' % base_name.title()
			else: description = title
			fanarts = xbmc.translatePath(os.path.join('special://home/addons/script.xxxodus.artwork', 'resources/art/%s/fanart.jpg' % filename))
			dirlst.append({'name': title, 'url': content_url, 'mode': player_mode, 'icon': icon, 'fanart': fanarts, 'description': description, 'folder': False})
		except Exception as e:
