...
def content(url,searched=False):
    ...
    try:
        c = client.request(url)
        r = dom_parser2.parse_dom(c, 'div', {'id': re.compile('boxVideo_.+?')})
        r = [i for i in r if '[[video_category]]' not in i.content]
        r = [(dom_parser2.parse_dom(i, 'a', req=['href','title']), \
              dom_parser2.parse_dom(i, 'div', {'class': 'video_duration'}), \
