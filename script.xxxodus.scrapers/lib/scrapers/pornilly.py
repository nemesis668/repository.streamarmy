...
def menu():
    ...
    try:
        ...
        r = dom_parser2.parse_dom(c, 'li', {'class': 'inline-block'})
        r = dom_parser2.parse_dom(r, 'a', req='href')
        r = [(urlparse.urljoin(base_domain,i.attrs['href']), i.content.title()) for i in r]
        if ( not r ):
...
def content(url,searched=False):
    ...
    try:
        c = client.request(url)
        r = dom_parser2.parse_dom(c, 'li', {'class': 'col-lg-3'})
        r = [(dom_parser2.parse_dom(i, 'a', req=['href','style']), \
              dom_parser2.parse_dom(i, 'h3'), \
              dom_parser2.parse_dom(i, 'span')) \
              for i in r if i]
        r = [(urlparse.urljoin(base_domain,i[0][0].attrs['href']), i[1][0].content, i[2][0].content, \
              re.findall('\(([^\)]+)', i[0][0].attrs['style'])[0]) for i in r]
    ...
    for i in r:
        try:
            ...
            name += ' - [ %s ]' % kodi.sortX(i[2].encode('utf-8'))
    ...
    if not searched:
        ...
        try:
            search_pattern = '''<li><a\s*href=['"]([^'"]+)['"]\s*rel="next">'''
            parse = base_domain        
