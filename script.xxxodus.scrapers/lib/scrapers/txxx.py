...
def menu():
	...
	try:
		...
		r = re.findall('<div class="c-thumb">(.*?)</div>',c,flags=re.DOTALL)
	...
	for i in r:
		try:
			...
			#number = re.findall('<span>(.*?)</span>',i,flags=re.DOTALL)[0]
...
def content(url,searched=False):
	...
	try:
		c = client.request(url)
		r = re.findall('<div class="un-grid--thumb--content">(.*?)<div class="un-grid--thumb--info">',c,flags=re.DOTALL)
	except Exception as e:
		if ( not searched ):
			log_utils.log('Fatal Error in %s:: Error: %s' % (base_name.title(),str(e)), log_utils.LOGERROR)
			kodi.notify(msg='Fatal Error', duration=4000, sound=True)
	...
	for i in r:
		try:
			...
			icon = re.findall('<img src="(.*?)"',i,flags=re.DOTALL)[0]
			#time = re.findall('<div class="thumb__duration">(.*?)</div>',i,flags=re.DOTALL)[0]
