...
def menu():
    ...
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'a', req=['href','title']), \
            dom_parser2.parse_dom(i, 'p', {'class': 'numberVideos'}), \
            dom_parser2.parse_dom(i, 'img', req='data-src')) \
            for i in r if i]
        r = [(urlparse.urljoin(base_domain,i[0][0].attrs['href']), i[0][0].attrs['title'], i[1][0].content.split(' ')[0], i[2][0].attrs['data-src']) for i in r]
        if ( not r ):
...
def content(url,searched=False):
    ...
    try:
        c = client.request(url)
        r = dom_parser2.parse_dom(c, 'div', {'class': 'video_block_wrapper'})
        r = [(dom_parser2.parse_dom(i, 'a', {'class': ['video_link','js_mpop']}), \
            dom_parser2.parse_dom(i, 'span', {'class': 'duration'}), \
            dom_parser2.parse_dom(i, 'img', req='data-mediumthumb'), \
            dom_parser2.parse_dom(i, 'div', {'class': 'video_title'})) \
            for i in r if i]
        r = [(urlparse.urljoin(base_domain,i[0][0].attrs['href']), i[3][0].attrs['title'], i[1][0].content, i[2][0].attrs['data-mediumthumb']) for i in r if i[0] and i[1] and i[2]]
